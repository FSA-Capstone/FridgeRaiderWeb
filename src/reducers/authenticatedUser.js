...
const saveRecipe = (recipe, userId) => {
	...
	return (dispatch, getState) => {
		return axios.put(`${process.env.API_URL}/api/users/${userId}/saveRecipe/${recipe.id}`).then(() => {
	...
	return async (dispatch) => {
        ...
        `,
			...
			if (response.data.name) {
				...
				let newUserResponse = await axios.post(`${process.env.API_URL}/api/users`, newUser);
